...
### sentinel-osint
Layer 2. FastAPI app running async collector workers — one file per data source,
all extending `BaseCollector`. Stores raw records in Postgres, links them via a
NetworkX identity graph, assembles `ProfileRecord` objects, and publishes
`PROFILE` events to the bus.

**Collectors:**

| Collector | Source | Requires key |
|---|---|---|
| `wigle` | WiGLE.net WiFi geolocation | Yes |
| `fitness` | Strava public route segments | Yes (OAuth) |
| `reviews` | Google Places reviewer profiles | Yes |
| `property` | County assessor open data | No |
| `username` | Cross-platform username existence | No |
| `shodan` | Shodan InternetDB — ports, CVEs by IP | No |
| `shodan_full` | Shodan full search API | Yes |
| `adsb_history` | ADS-B Exchange archived flight tracks | Yes |
| `aircraft_reg` | FAA aircraft registry | No |
| `marine_traffic` | MarineTraffic vessel data | No |
| `companies` | OpenCorporates company registrations | No |
| `companies_house` | Companies House UK | No |
| `sec_edgar` | SEC EDGAR US filings | No |
| `hunter` | Hunter.io professional emails | Yes |
| `pastes` | Pastebin / GitHub gist leak search | No |
| `flight_reg` | FlightAware / FR24 aircraft registration | Yes |
