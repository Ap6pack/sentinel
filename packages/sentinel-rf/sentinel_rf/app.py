...
from .publisher import SyncRFPublisher
...
def create_app() -> tuple[Flask, SocketIO]:
    ...
    app.config["SECRET_KEY"] = os.environ.get(
        "SENTINEL_JWT_SECRET", os.urandom(32).hex()
    )
...
async def run_decoders(publisher: SyncRFPublisher) -> None:
    """Start all configured decoders and publish events via sync publisher."""
...
def _run_decoder_thread(publisher: SyncRFPublisher) -> None:
    """Entry point for the decoder background thread."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(run_decoders(publisher))
...
def main() -> None:
    ...
    publisher = SyncRFPublisher(redis_url=settings.redis_url)

    t = threading.Thread(target=_run_decoder_thread, args=(publisher,), daemon=True)
    t.start()
